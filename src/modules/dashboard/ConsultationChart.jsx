...
const ConsultationChart = () => {
  ...
  // Generate list of last 13 months to display on X axis
  // e.g. Jun/25 to Jun/26
  const getMonthsList = () => {
    const list = [];
    const monthsNames = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];
    
    // We want to construct months from June 2025 to June 2026 (13 months)
    let startYear = 2025;
    ...
    for (let i = 0; i < 13; i++) {
      list.push({
        ...
        month: startMonthIdx + 1, // 1-indexed
        year: startYear
      });
  ...
  const chartData = months.map(m => {
    const count = appointments.filter(ap => {
      ...
      // Verify UTC or local compatibility safely
