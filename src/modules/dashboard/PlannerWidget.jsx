...
const PlannerWidget = () => {
  ...
  const [currentDate, setCurrentDate] = useState(new Date('2026-06-19')); // Standard initial date from prints
  ...
  return (
    <div className="card">
      ...
      <button className="btn-teal" onClick={() => setIsOpen => setIsAdding(!isAdding)} style={{ marginBottom: '16px' }}>
        + adicionar tarefa
      </button>
      ...
      {isAdding && (
        <form onSubmit={handleAddTask} style={{ marginBottom: '16px', display: 'flex', gap: '8px' }}>
          ...
          <button type="button" className="btn-teal" style={{ width: 'auto', padding: '0 16px', backgroundColor: 'var(--bg-card-hover)', border: '1px solid rgba(255,255,255,0.1)' }} onClick={() => setIsAdding(false)}>X</button>
        </form>
      )}
