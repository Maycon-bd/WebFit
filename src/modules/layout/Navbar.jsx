...
const Navbar = ({ onOpenProfile }) => {
  ...
  const handleQuickAction = (action) => {
    ...
    if (action === 'task') {
      ...
      const title = prompt('Digite o título do novo questionário / modelo:');
      const content = prompt('Digite o conteúdo / perguntas do questionário:');
      if (title && content) {
        setMessageTemplates(prev => [
          ...prev, 
          { id: Date.now().toString(), title: title.trim(), content: content.trim() }
        ]);
        alert('Novo questionário cadastrado com sucesso nos Modelos de Mensagens!');
      }
    } else if (action === 'cloud') {
      const fileName = prompt('Digite o nome do arquivo para subir na nuvem (Cloud):');
      if (fileName && fileName.trim() !== '') {
        alert(`[Cloud] Arquivo "${fileName.trim()}" enviado com sucesso para o armazenamento em nuvem do WebFit!`);
      }
    }
  };
